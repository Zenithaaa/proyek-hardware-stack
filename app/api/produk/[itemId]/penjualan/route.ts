...
function getStartDate(period: ChartPeriod, referenceDate: Date = new Date()): Date {
...
function getWeekLabel(date: Date): string {
  ...
  return `W${weekNum.toString().padStart(2, '0')} '${d.getUTCFullYear().toString().slice(-2)}`;
}
...
export async function GET(
  ...
  if (!period || !["daily", "weekly", "monthly"].includes(period)) {
    return NextResponse.json(
      { message: "Invalid or missing period parameter. Choose from daily, weekly, monthly." },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    salesRecords.forEach((record) => {
      if (!record.transaksi || !record.transaksi.tanggalTransaksi) {
        console.warn("Skipping sales record due to missing transaction date:", record);
        return;
      }
      ...
      switch (period) {
        ...
        case "weekly":
          ...
          break;
        case "monthly":
          label = getMonthLabel(new Date(saleDate.getFullYear(), saleDate.getMonth(), 1));
          break;
      }
    ...
    if (period === "daily") {
        ...
        Array.from(distinctLabelsInSales)
            .sort((a,b) => new Date(a.split(" ")[1] + " " + a.split(" ")[0] + " " + endDate.getFullYear()).getTime() - new Date(b.split(" ")[1] + " " + b.split(" ")[0] + " " + endDate.getFullYear()).getTime())
            .forEach(label => {
            if(addedLabelsCount < 7) {
                formattedChartData.push({ periodLabel: label, sales: aggregatedSales[label] || 0 });
                addedLabelsCount++;
            }
        });

        Array.from(allLabelsInRange).forEach(label => {
            if(addedLabelsCount < 7 && !formattedChartData.find(p => p.periodLabel === label)) {
                formattedChartData.push({ periodLabel: label, sales: 0 });
                addedLabelsCount++;
            }
        });
        formattedChartData.sort((a, b) => {
            const dateA = new Date(a.periodLabel.split(" ")[1] + " " + a.periodLabel.split(" ")[0] + " " + endDate.getFullYear());
            const dateB = new Date(b.periodLabel.split(" ")[1] + " " + b.periodLabel.split(" ")[0] + " " + endDate.getFullYear());
            return dateA.getTime() - dateB.getTime();
        });
        if(formattedChartData.length > 7) formattedChartData.splice(0, formattedChartData.length - 7);
        ...
        Array.from(distinctLabelsInSales)
            .sort((a,b) => {
                const weekA = parseInt(a.substring(1,3)); const yearA = parseInt(a.substring(5,7));
                const weekB = parseInt(b.substring(1,3)); const yearB = parseInt(b.substring(5,7));
                if(yearA !== yearB) return yearA - yearB; return weekA - weekB;
            })
            .forEach(label => {
        ...
        Array.from(allLabelsInRange).forEach(label => {
            if(addedLabelsCount < 4 && !formattedChartData.find(p => p.periodLabel === label)) {
                formattedChartData.push({ periodLabel: label, sales: 0 });
                addedLabelsCount++;
            }
        });
        formattedChartData.sort((a, b) => {
            const weekA = parseInt(a.periodLabel.substring(1, 3)); const yearA = parseInt(a.periodLabel.substring(5, 7));
            const weekB = parseInt(b.periodLabel.substring(1, 3)); const yearB = parseInt(b.periodLabel.substring(5, 7));
            if (yearA !== yearB) return yearA - yearB; return weekA - weekB;
        });
        if(formattedChartData.length > 4) formattedChartData.splice(0, formattedChartData.length - 4);
        ...
        const allLabelsInRange = new Set<string>();
         for (let m = 0; m < 6; m++) {
            ...
            if (monthDate < new Date(new Date(startDate).setDate(1)) && m > 0) break; 
        ...
        Array.from(distinctLabelsInSales)
            .sort((a,b) => {
                const [mAS, yAS] = a.split(" '"); const dA = new Date(Date.parse(mAS +" 1, 20"+yAS));
                const [mBS, yBS] = b.split(" '"); const dB = new Date(Date.parse(mBS +" 1, 20"+yBS));
        ...
        Array.from(allLabelsInRange).reverse().forEach(label => {
            if(addedLabelsCount < 6 && !formattedChartData.find(p => p.periodLabel === label)) {
                 formattedChartData.push({ periodLabel: label, sales: 0 });
                 addedLabelsCount++;
            }
        });
        formattedChartData.sort((a, b) => {
            const [mAS, yAS] = a.periodLabel.split(" '"); const dA = new Date(Date.parse(mAS +" 1, 20"+yAS));
            const [mBS, yBS] = b.periodLabel.split(" '"); const dB = new Date(Date.parse(mBS +" 1, 20"+yBS));
            return dA.getTime() - dB.getTime();
        });
        if(formattedChartData.length > 6) formattedChartData.splice(0, formattedChartData.length - 6);
    }
    ...
    console.error(`Error fetching sales data for item ${itemId}, period ${period}:`, error);
